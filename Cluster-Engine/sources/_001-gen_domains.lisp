...
(defmethod pitchdomain-range ((min integer) (step integer) (max integer))
  (mapcar #'list (om::arithm-ser min max step)));(pw::arithm-ser min step max)))

(defmethod pitchdomain-range ((min list) (step integer) (max list))
  (om::mat-trans (loop for i in min ;(pw::mat-trans (loop for i in min
        for j in max
    collect (om::arithm-ser i j step ))));(pw::arithm-ser i step j))))

(defmethod pitchdomain-range-chords ((min list) (step integer) (max list))
  (mapcar #'list (om::mat-trans (loop for i in min ;(pw::mat-trans (loop for i in min
                                  for j in max
                                  collect (om::arithm-ser i j step)))));(pw::arithm-ser i step j)))))
